...
public class MemberController {
    ...
    public String detail(@RequestParam Long id, Model model) {
        model.addAttribute("member", MemberResponse.from(memberService.findById(id)));
    ...
    // 회원 수정 처리
    @PostMapping("/edit")
    public String edit(@RequestParam Long id, @ModelAttribute MemberRequest.Update request, RedirectAttributes redirectAttributes) {
        try {
            memberService.update(id, request.getName(), request.getEmail());
            redirectAttributes.addFlashAttribute("message", "회원 정보가 수정되었습니다.");
            return "redirect:/members/detail?id=" + id;
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("error", e.getMessage());
            return "redirect:/members/edit?id=" + id;
        }
    }
